    ...
    def T(f1,f2,g1,g2):  
        ...
        for i in range(2**N):
            for j in range(2**N):
                ...
                result += S_f(i/Lx - 2**(N-1)/Lx, j/Lx - 2**(N-1)/Lx)*pupil2*np.conjugate(pupil2)
        return result
    ...
    def TT():
        ...
        for i in range(2**(2*N)):
            for j in range(2**(2*N)):
                TT[i][j] = T(fx[i-2**N*(ceil(i//2**N))], fx[ceil(i//2**N)], fx[j-2**N*(ceil(j//2**N))], fx[ceil(j//2**N)])
        return TT
    TCC = TT()
    return TCC
TCC = tcc()
plot(np.real(TCC),"TCC")
