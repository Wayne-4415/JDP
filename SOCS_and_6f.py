...
for i in range(2**N):
    for j in range(2**N):
        if S[i][j] != 0:
            I_6f += ift(P_double[i:2**N+i, j:2**N+j] * T, x, fx)
            normalize += 1
        else:
            continue
I_6f = I_6f/normalize
...
while s[state] > 0.01 * s[0]:
    I_SOCS += np.real(np.square(np.absolute(ift(np.reshape(vh[state,:], (2**N,2**N)) * T, x, fx))) * s[state])
    state += 1
...
err = np.square(np.absolute(I_SOCS - I_6f))
