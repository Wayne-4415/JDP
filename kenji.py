...
def ift(f,x,fx):
    ...
    for i in range(len(x)):
        for j in range(len(x)):
            temp[i,j] = np.sum(f*np.exp(1j*2*pi*(fxx*x[i]+fyy*x[j])))
    return temp
...
for i in range(2**N):
    for j in range(2**N):
        if (abs(fx[i])-0.5)**2+(abs(fx[j])-0.6)**2<0.017:
            S[i,j] = 1
        if (abs(fx[i])-0.6)**2+(abs(fx[j])-0.5)**2<0.017:
            S[i,j] = 1
...
for i in range(3):
    ...
    # eig_I[i,:,:] = fold(np.fft.ifft2(phi[i,:,:]))
